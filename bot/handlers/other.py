        ...
        case 'request':
            ...
            for admin in admins + main_admins:
                try:
                    await tg_bot.send_message(
                        admin.tg_user_id,
                        f'User @{callback.from_user.username} ({callback.from_user.id}) просит помощи',
                        reply_markup=inline_markup_credentials(callback.from_user.id)
                    )
                except Exception as exc:
...
async def callback__(*args, ** kwargs):
    ...
    message_text = f'<code>{message_text}</code>'
    main_admins = get_users(user_group='m_admin')
    logger.error(f'Unknown callback from @{callback.from_user.username} (callback.from_user.id), data: {callback.data}')
