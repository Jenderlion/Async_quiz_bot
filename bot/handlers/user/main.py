...
async def command_start(message: aio_types.Message, user: User):
    answer_text = f'Привет! Я автоматизированная система, созданная для проведения анкетирования новых и постоянных' \
                  f' клиентов ОАО «ГУМ».\n' \
                  f'Основная задача — определить целевую аудиторию ОАО «ГУМ», составить портрет покупателя, узнать' \
                  f' его предпочтения и пожелания. На основе данной информации расширить ассортимент товаров,' \
                  f' оптимизировать скидочную систему, улучшить обслуживание. Для достижения этих целей предлагаю' \
                  f' Вам заполнить анкету.\n\n' \
